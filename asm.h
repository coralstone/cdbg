...
typedef struct _addrdec {
  int            defseg;
  const char    *descr;
//  _addrdec() {defseg=0;descr=0;}
} t_addrdec;
...
// their properties.
/*
ext const char      *regname[3][9];
ext const char      *segname[8];
ext const char      *sizename[11];
ext const t_addrdec addr16[8];
ext const t_addrdec addr32[8];
ext const char      *fpuname[9];
ext const char      *mmxname[9];
ext const char      *crname[9];
ext const char      *drname[9];
ext const char      *condition[16];
ext const t_cmddata cmddata[1];
ext const t_cmddata vxdcmd;
ext const t_cmddata dangerous[1];
*/
...
typedef struct _dasm {                 // Results of disassembling
  ulong          ip;                   // Instrucion pointer
  char           dump[TEXTLEN];         // Hexadecimal dump of the command
  char           text[TEXTLEN];        // Disassembled command
...
typedef struct _masm {            // Model to search for assembler command
...
ext int       ideal;                // Force IDEAL decoding mode
ext int       lowercase;            // Force lowercase display
ext int       tabarguments;         // Tab between mnemonic and arguments
ext int       extraspace;           // Extra space between arguments
ext int       putdefseg;            // Display default segments in listing
ext int       showmemsize;          // Always show memory size
ext int       shownear;             // Show NEAR modifiers
ext int       shortstringcmds;      // Use short form of string commands
ext int       sizesens;             // How to decode size-sensitive mnemonics
ext int       symbolic;             // Show symbolic addresses in disasm
ext int       farcalls;             // Accept far calls, returns & addresses
ext int       decodevxd;            // Decode VxD calls (Win95/98)
ext int       privileged;           // Accept privileged commands
ext int       iocommand;            // Accept I/O commands
ext int       badshift;             // Accept shift out of range 1..31
ext int       extraprefix;          // Accept superfluous prefixes
ext int       lockedbus;            // Accept LOCK prefixes
ext int       stackalign;           // Accept unaligned stack operations
ext int       iswindowsnt;          // When checking for dangers, assume NT

int    EncAsm(const char *cmd,ulong len,
                ulong addr,
                t_masm *model,int attempt,int constsize);

ulong  DecAsm(const void *buff,ulong len,
              ulong addr,
              t_dasm *disasm,int disasmmode);



/*
int    CheckCondition(int code,ulong flags);
int    DecAddress(ulong addr,char *symb,int nsymb,char *comment);

ulong  DisBack(char *block,ulong base,ulong size,ulong ip,int n);
ulong  DisForward(char *block,ulong base,ulong size,ulong ip,int n);
