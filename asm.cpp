...
static const char  *asmcmd;              // Pointer to 0-terminated source line
...
static const char      *asmerror;            // Explanation of last error, or NULL
...
static void  sasm_error(const char* err) {asmerror = err; scan = SCAN_ERR;}
#define RETURN_ERROR(err) do{  sasm_error(err); return; }while(0)
...
static void Scanasm(int mode) {
  ...
  if (isalpha(*asmcmd) || *asmcmd=='_' || *asmcmd=='@') {
    ...
    while ((isalnum(*asmcmd) || *asmcmd=='_' || *asmcmd=='@') &&
      ...
      sdata[i++]=*asmcmd++;
    if (i>=sizeof(sdata)) {
      RETURN_ERROR("Too long identifier");
      };
    sdata[i]='\0';
    while (*asmcmd==' ' || *asmcmd=='\t')
    ...
    if (strcmp(s,"ST")==0) {
      ...
      if ((scan!=SCAN_ICONST && scan!=SCAN_DCONST) || idata<0 || idata>7) {
        RETURN_ERROR("FPU registers have indexes 0 to 7");
            };
      Scanasm(SA_NAME);
      if (scan!=SCAN_SYMB || idata!=')') {
        RETURN_ERROR("Closing parenthesis expected");
            };
    ...
    if (strcmp(s,"LOCAL")==0 && *asmcmd=='.') {
      ...
      while (*asmcmd==' ' || *asmcmd=='\t')
        asmcmd++;                      // Skip trailing spaces
      if (!isdigit(*asmcmd)) {
        RETURN_ERROR("Integer number expected");
           };
    ...
    if (mode & SA_NAME) {
      ...
      return; }
    RETURN_ERROR("Unknown identifier");
            }
    ...
    if (maxdigit==0) {
      RETURN_ERROR("Hexadecimal digits after 0x... expected");
            };
    if (toupper(*asmcmd)=='H') {       // Force hexadecimal number
      if (base==16) {
        RETURN_ERROR("Please don't mix 0xXXXX and XXXXh forms");
            };
      asmcmd++;
    ...
    if (*asmcmd=='.') {                // Force decimal number
      if (base==16 || maxdigit>9) {
        RETURN_ERROR("Not a decimal number");
            };
      asmcmd++;
      if (isdigit(*asmcmd) || toupper(*asmcmd)=='E') {
        ...
        if (toupper(*asmcmd)=='E') {
          ...
          if (!isdigit(*asmcmd)) {
            RETURN_ERROR("Invalid exponent");
           };
          decimal=0;
          while (isdigit(*asmcmd)) {
            if (decimal<65536L) decimal=decimal*10+(*asmcmd++)-'0';
            };
          floating*= pow(10,decimal*base);//pow10l(decimal*base);
          };
        fdata=floating;
    ...
    if (*asmcmd=='\0' || (*asmcmd=='\\' && asmcmd[1]=='\0'))  {
      RETURN_ERROR("Unterminated character constant");
            };
    if (*asmcmd=='\'') {
      RETURN_ERROR("Empty character constant");
            };
    if (*asmcmd=='\\') asmcmd++;
    idata=*asmcmd++;
    if (*asmcmd!='\'')  {
    ...
    if (idata=='|' && *asmcmd=='|') {
      ...
      if (*asmcmd=='&') {              // Import pseudolabel (for internal use)
        ...
        while (*asmcmd!='\0' && *asmcmd!='>') {
          sdata[i++]=*asmcmd++;
          if (i>=sizeof(sdata)) {
            RETURN_ERROR("Too long import name");
          }
        };
        if (*asmcmd!='>') {
          RETURN_ERROR("Unterminated import name");
        }
        asmcmd++; sdata[i]='\0';
...
static void Parseasmoperand(t_asmoperand *op) {
  ...
  if (scan==SCAN_JMPSIZE) {
    ...
    if (
      ...
      RETURN_ERROR("Invalid combination of jump address modifiers");
            };
  ...
  if (scan==SCAN_REG8 || scan==SCAN_REG16 || scan==SCAN_REG32) {
    ...
    if (scan==SCAN_SYMB && idata==':') {
      ...
      if (scan!=SCAN_ICONST && scan!=SCAN_DCONST && scan!=SCAN_OFS) {
        RETURN_ERROR("Integer address expected");
          };
    ...
    RETURN_ERROR("Floating-point numbers are not allowed in command");
          }
  // Segment register or address.
    ...
    while (1) {
      if (scan==SCAN_SYMB && idata=='[') {
        if (bracket) {                 // Bracket
          RETURN_ERROR("Only one opening bracket allowed");
          };
        bracket=1; }
      else if (scan==SCAN_OPSIZE) {
        if (op->size!=0) {             // Size of operand
          RETURN_ERROR("Duplicated size modifier");
          };
        op->size=idata; }
      else if (scan==SCAN_SEG) {
        if (op->segment!=SEG_UNDEF) {  // Segment register
          RETURN_ERROR("Duplicated segment register");
          };
        op->segment=idata; Scanasm(0);
        if (scan!=SCAN_SYMB || idata!=':') {
          RETURN_ERROR("Semicolon expected");
        };
      }
      ...
      Scanasm(0); };
    if (bracket==0) {
      RETURN_ERROR("Address expression requires brackets");
          };
    ...
    while (1) {                        // Get SIB and offset
      ...
      if (scan==SCAN_REG8 && idata==REG_EAX) {
        if (sign=='-') {
          RETURN_ERROR("Unable to subtract register");
          };
        if (xlataddr!=0) {
          RETURN_ERROR("Too many registers");
          };
        xlataddr=1;
        Scanasm(0); }
      else if (scan==SCAN_REG16) {
        RETURN_ERROR("Sorry, 16-bit addressing is not supported");
          }
        ...
        if (scan==SCAN_SYMB && idata=='*') {
          ...
          if (scan==SCAN_ERR) return;
          if (scan==SCAN_OFS) {
            RETURN_ERROR("Undefined scale is not allowed");
          };
          if (scan!=SCAN_ICONST && scan!=SCAN_DCONST) {
            RETURN_ERROR("Syntax error");
          };
          if (idata==6 || idata==7 || idata>9) {
            RETURN_ERROR("Invalid scale");
          };
          r[reg]+=idata;
          Scanasm(0); }
      ...
      for (i=0; i<=8; i++) {
        ...
        if (r[i]==3 || r[i]==5 || r[i]==9) {
          if (op->index>=0 || op->base>=0) {
            RETURN_ERROR( j==0 ?"Invalid scale":"Too many registers");
             };
          op->index=op->base=i;
          op->scale=r[i]-1; }
        else if (r[i]==2 || r[i]==4 || r[i]==8) {
          if (op->index>=0) {
           RETURN_ERROR(j<=1 ?"Only one register may be scaled":"Too many registers");
            };
    ...
    RETURN_ERROR("Unrecognized operand");
  ...
  if (op->jmpmode!=0 && op->type!=IMM && op->type!=JMF && op->type!=MRG) {
    RETURN_ERROR("Jump address modifier is not allowed");
     };
...
static void  masm_error(const char* err,t_masm *masm) {strcpy(masm->errtext,err);}
#ifdef RETURN_ERROR
#undef RETURN_ERROR
#define RETURN_ERROR(err) do{  masm_error(err,model);  goto error ; }while(0)
#endif // RETURN_ERROR


int EncAsm(const char *cmd, ulong len, ulong addr,t_masm *model,int attempt,int constsize )
{
  ...
  while (1) {                          // Fetch all REPxx and LOCK prefixes
    if (scan==SCAN_REP || scan==SCAN_REPE || scan==SCAN_REPNE) {
      if (rep!=0) {
        RETURN_ERROR("Duplicated REP prefix");
      };
      rep=scan; }
    else if (scan==SCAN_LOCK) {
      if (lock!=0) {
        RETURN_ERROR("Duplicated LOCK prefix");
      };
      lock=scan; }
    else break;                        // No more prefixes
    Scanasm(SA_NAME); };
  if (scan!=SCAN_NAME || idata>16) {
         RETURN_ERROR("Command mnemonic expected");
  };
  nameend=asmcmd;
  strupr(sdata);
  ...
  if (scan==SCAN_ERR) {
    RETURN_ERROR(asmerror);
  };
  if (scan!=SCAN_EOL) {
    RETURN_ERROR("Extra input after operand");
   };
...
retrylongjump:
    ...
    for (j=0; j<3; j++) {              // Up to 3 operands
      ...
      switch (arg) {
        ...
        default:                       // Undefined type of operand
          RETURN_ERROR("Internal Assembler error");
        goto error;
      };                               // End of switch (arg)
  ...
  if (match!=0) {                      // Command not found
    if (minop>0 && aop[minop-1].type==NNN)
      RETURN_ERROR("Too few operands");
    else if (maxop<3 && aop[maxop].type!=NNN)
      RETURN_ERROR("Too many operands");
    else if (nameok>1)                 // More that 1 command
      RETURN_ERROR("Command does not support given operands");
    else if (match & MA_JMP)
      RETURN_ERROR("Invalid jump size modifier");
    else if (match & MA_NOP)
      RETURN_ERROR("Wrong number of operands");
    else if (match & MA_TYP)
      RETURN_ERROR("Command does not support given operands");
    else if (match & MA_NOS)
      RETURN_ERROR("Please specify operand size");
    else if (match & MA_SIZ)
      RETURN_ERROR("Bad operand size");
    else if (match & MA_DIF)
      RETURN_ERROR("Different size of operands");
    else if (match & MA_SEG)
      RETURN_ERROR("Invalid segment register");
    else if (match & MA_RNG)
      RETURN_ERROR("Constant out of expected range");
    else
      RETURN_ERROR("Erroneous command");
    goto error;
  };
  ...
  *(ulong *)tmask=pd->mask;
  ...
  if (pd->bits==WW || pd->bits==WS || pd->bits==WP) {
    ...
    if (datasize==0) {
      RETURN_ERROR("Please specify operand len");
       }
    else if (datasize>1)
      ...
      case MFX:                        // Memory in ModRM byte (ext. FPU state)
        ...
        if (op->base<0 && op->index<0) {
          ...
          if (op->index==REG_ESP) {    // Reordering was unsuccessfull
            RETURN_ERROR("Invalid indexing mode");
              };
          if (op->base<0) {
            tcode[i+1]|=0x04;
            dispsize=4; }
  ...
  if (immsize>0) {
    ...
    if ((immediate & l)!=0 && (immediate & l)!=l) {
      RETURN_ERROR("Constant does not fit into operand");
        };
  ...
  if (jmpsize!=0) {
    if (addr!=0) {
      jmpoffset=jmpoffset-addr;
      if (jmpsize==1 && anyjmp==0 && (jmpoffset<-128 || jmpoffset>=128)) {
        if (longjump==0 && (jmpmode & 0x03)==0) {
          longjump=1;
          goto retrylongjump;
        };
        RETURN_ERROR("Relative jump out of range, use  LONG form" );
      };
  ...
  if(i>0)
  {
     int n = 0,k=0;
     for (;k<i;k++)
       n += sprintf(model->dump+n,"%02X ",(uchar)model->code[k]);
  }
  i+=j;
  model->length=i;
  return i;
...
static t_dasm *da;                  // Pointer to disassembly results
...
static void DecodeRG(int index,int datasize,int type) {
  ...
  if (mode>=DISASM_FILE) {
    ...
    if (type<PSEUDOOP)                 // Not a pseudooperand
      nresult+=sprintf(da->text+nresult,"%s",name);
    ;
  };
};
...
static void Memadr(int defseg,const char *descr,long offset,int dsize) {
  ...
  if (offset==0L) {
    ...
    if (symbolic && mode>=DISASM_CODE)
      i=DecAddress(offset,s,TEXTLEN-n-24,NULL);
    else i=0;
    ...
    switch (type) {
      ...
      case MRJ:                        // Memory/reg in ModRM as JUMP target
        ...
        if (mode>=DISASM_FILE && shownear!=0)
          nresult+=sprintf(da->text+nresult,"%s ",(lowercase?"near":"NEAR"));
        break;
      case MR1:                        // 1-byte memory/register in ModRM byte
        dsize=regsize=1;
      ...
      case MMS:                        // Memory in ModRM byte (as SEG:OFFS)
        ...
        if (datasize==4 && inmemory)
          da->memtype=DEC_FWORD;
        if (mode>=DISASM_FILE)
          nresult+=sprintf(da->text+nresult,"%s ",(lowercase?"far":"FAR"));
        break;
      case MM6:                        // Memory in ModRM (6-byte descriptor)
        dsize=6; memonly=1;
...
static void DecodeIM(int constsize,int sxt,int type) {
  ...
  if (mode>=DISASM_FILE && da->error==DAE_NOERR) {
    if (mode>=DISASM_CODE && type!=IMU)
      i=DecAddress(data,name,TEXTLEN-nresult-24,comment);
    else {
      i=0; comment[0]='\0'; };
    if (i!=0 && symbolic!=0) {
      strcpy(da->text+nresult,name); nresult+=i; }
    else if (type==IMU || type==IMS || type==IM2 || data>=0 || data<NEGLIMIT)
      nresult+=sprintf(da->text+nresult,"%lX",data);
    else
      nresult+=sprintf(da->text+nresult,"-%lX",-data);
...
static void DecodeVX(void) {
  ...
  if (da->immconst==0)
    da->immconst=data;
  if (mode>=DISASM_FILE && da->error==DAE_NOERR) {
    if ((data & 0x00008000)!=0 && memicmp("VxDCall",da->text,7)==0)
      memcpy(da->text,lowercase?"vxdjump":"VxDJump",7);
    nresult+=sprintf(da->text+nresult,"%lX",data);
  };
};
...
static void DecodeC1(void) {
  if (mode<DISASM_DATA) return;
  da->immconst=1;
  if (mode>=DISASM_FILE) nresult+=sprintf(da->text+nresult,"1");
};
...
static void DecodeRJ(ulong offsize,ulong nextip) {
  ...
  if (addr==0) da->zeroconst=1;
  if (mode>=DISASM_FILE) {
    if (offsize==1) nresult+=sprintf(da->text+nresult,
      "%s ",(lowercase==0?"SHORT":"short"));
    if (mode>=DISASM_CODE)
      i=DecAddress(addr,s,TEXTLEN,da->comment);
    else
      i=0;
    if (symbolic==0 || i==0)
      nresult+=sprintf(da->text+nresult,"%08lX",addr);
    else
      nresult+=sprintf(da->text+nresult,"%.*s",TEXTLEN-nresult-25,s);
...
static void DecodeJF(void) {
  ...
  if (mode>=DISASM_FILE) {
    nresult+=sprintf(da->text+nresult,"%s %04X:%08X",
...
static void DecodeSG(int index) {
  ...
  if (mode>=DISASM_FILE) {
    i=sprintf(da->text+nresult,"%s",segname[index]);
    if (lowercase) strlwr(da->text+nresult);
    nresult+=i;
  };
};
...
static void DecodeCR(int index) {
  ...
  if (mode>=DISASM_FILE) {
    index=(index>>3) & 0x07;
    nresult+=sprintf(da->text+nresult,"%s",crname[index]);
    if (lowercase) strlwr(da->text+nresult);
  };
};
...
static void DecodeDR(int index) {
  ...
  if (mode>=DISASM_FILE) {
    index=(index>>3) & 0x07;
    i=sprintf(da->text+nresult,"%s",drname[index]);
    if (lowercase) strlwr(da->text+nresult);
    nresult+=i;
  };
};
...
ulong DecAsm(const void *buff,ulong len,ulong addr,t_dasm *disasm,int disasmmode)
{
  ...
  if (repeated) {
    if (mode>=DISASM_FILE) {
      ...
      nresult+=sprintf(da->text+nresult,"PREFIX %s:",pname);
      if (lowercase) strlwr(da->text);
  ...
  if (lockprefix!=0) {
    if (mode>=DISASM_FILE) nresult+=sprintf(da->text+nresult,"LOCK ");
  ...
  if (pd->mask==0) {                   // Command not found
    ...
    if (mode>=DISASM_FILE) {
      ...
      if (repprefix!=0 && tabarguments) {
        for (i=0; name[i]!='\0' && name[i]!=' '; i++)
          da->text[nresult++]=name[i];
        if (name[i]==' ') {
          da->text[nresult++]=' '; i++; };
        while (nresult<8) da->text[nresult++]=' ';
        for ( ; name[i]!='\0'; i++)
          da->text[nresult++]=name[i];
        ; }
      else
    ...
    for (operand=0; operand<3; operand++) {
      ...
      switch (arg) {
        ...
        case IMA:                      // Immediate absolute near data address
          DecodeIA(); break;
        case JOB:                      // Immediate byte offset (for jumps)
          DecodeRJ(1,addr+2); break;
        case JOW:                      // Immediate full offset (for jumps)
          DecodeRJ(datasize,addr+datasize+1); break;
        case JMF:                      // Immediate absolute far jump/call addr
          DecodeJF();
          da->warnings|=DAW_FARADDR; break;
        case SGM:                      // Segment register in ModRM byte
    ...
    if (pfixup!=NULL && da->fixupsize>0)
      da->fixupoffset=pfixup-(uchar*)buff;
  ...
  return (len-size);               // Returns number of recognized bytes
};
